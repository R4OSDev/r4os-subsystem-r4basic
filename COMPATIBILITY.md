...
selector. IDE, editor, native compiler, linker, and librarian behavior remain
separate products; the interpreter language and runtime are complete without
them.
...
(`QB45-ERR-*`). Every Part-1, Part-2, metacommand and error target is marked
implemented. The repository test rejects any other lexer, syntax, binder, VM,
error, coverage, or runtime-error status, so the catalog cannot silently
reopen.

The lexer performs lexical analysis only. The compiler Builder is the single
parser, binder, and emitter. Every catalogued construct binds to typed
bytecode or is rejected with a precise source diagnostic; there is no generic
runtime fallback for recognized language syntax.
...
4. Permanent general R4BASIC fixtures and inline reference vectors make the
   complete surface executable as a permanent test contract.
...
- A simple identifier starts with an ASCII letter, continues with ASCII
  ...
  field access. `_` is not an identifier character.
...
Every row below is an executable contract promise. `positive_*` fixtures demonstrate accepted
...
| ID | Syntax/runtime promise | Required by local acceptance | Positive fixture | Negative fixture |
| --- | --- | --- | --- | --- |
...
| SRC-02 | Apostrophe and `REM` comments; `$INCLUDE`, `$DYNAMIC`, and `$STATIC` metacommands | yes | `positive_source_contract.bas` plus inline source-graph and allocation-mode vectors | `negative_lexical.bas` |
...
| DEBUG-01 | Cooperative `STOP`, productive host continuation, and visible bounded `TRON`/`TROFF` statement tracing | yes | inline pause, reset, cancellation, host-input, and 256-entry ring vectors | inline lifecycle vectors |
...
| EXPR-02 | All catalogued numeric conversion, IEEE/MBF byte conversion, math, string, time, graphics and host-query built-ins | yes | `positive_io_graphics.bas` and inline numeric/byte/error vectors | `negative_expressions.bas` |
...
| DEVICE-01 | `OPEN COM`, COM events, `IOCTL`/`IOCTL$`, partial bounded RX/TX, `LPRINT`/`LPRINT USING`, `LPOS`, and screen/file/device/printer `WIDTH` | yes | inline 0.70.13 serial, partial-input, event, spool, formatting and ERDEV vectors | inline timeout, unavailable, overflow, invalid-option and out-of-paper vectors |
| ERROR-01 | All 43 Appendix-B error numbers preserve exact `ERR`/`ERL` identity and are resumable through the central handler model | yes | inline all-number round-trip vector plus natural language, file and device faults | inline nested-handler and invalid-resume vectors |
...
Wrong arities are syntax diagnostics before runtime behavior.
...
- Reserved words outside the QuickBASIC-4.5 interpreter contract receive
  explicit diagnostics. Invalid bytes or malformed grammar never disappear
  as comments or successful empty input.

## Product boundary

- The QuickBASIC 4.5 interpreter language and runtime described by all 193
  Part-2 entries, seven fundamentals, three metacommands and 43 Appendix-B
  errors are executable.
- Historical DOS and hardware surfaces use documented private guest memory,
  virtual devices, or typed R4OS facades. Unrestricted host memory, kernel,
  port, interrupt, serial, or printer access is intentionally impossible.
- QBX-only extensions, a source IDE, interactive debugger, native-code
  compiler, linker, librarian, and editor are separate possible products and
  are not required for interpreter completeness.
- Parse or bind success alone never counts as runtime correctness; the
  positive, negative, error, isolation, lifecycle, performance, Gorilla and
  productive image acceptances jointly define release readiness.
