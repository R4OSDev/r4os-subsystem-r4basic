...
function Test-R4BasicPerformanceMarker {
    ...
    foreach ($match in $matches) {
        ...
        if ($record.MaximumSlice -gt 262144) { throw "R4BASIC slice $($record.MaximumSlice) exceeds the 262144-instruction contract: $name." }
        if ($record.NoFixedSleep -ne 1) { throw "R4BASIC runnable guest still advertises a fixed wait: $name." }
        if ($record.ClockReads -eq 0 -or $record.MaximumClockReads -gt 20) { throw "R4BASIC clock-read bound is invalid: $name." }
...
if ($SelfTest) {
    $valid = @(
        'R4BASIC performance-workload: name=numeric instructions=90000 ticks=1000 ips=90000 slices=30 maxSlice=90000 timeLimited=2 noFixedSleep=1 clockReads=400 maxClockReads=18 nsPerInstruction=1100 stringClones=0 stringCloneBytes=0 borrowedBuiltins=0 ownedBuiltins=0 procedureCalls=0 localPoolGrows=0 localPoolReuses=0 localInitializations=0 result=OK',
        'R4BASIC performance-workload: name=string-assign instructions=80000 ticks=1000 ips=80000 slices=30 maxSlice=80000 timeLimited=2 noFixedSleep=1 clockReads=400 maxClockReads=18 nsPerInstruction=1200 stringClones=100 stringCloneBytes=409600 borrowedBuiltins=0 ownedBuiltins=1 procedureCalls=0 localPoolGrows=0 localPoolReuses=0 localInitializations=0 result=OK',
        'R4BASIC performance-workload: name=string-len instructions=120000 ticks=1000 ips=120000 slices=30 maxSlice=120000 timeLimited=2 noFixedSleep=1 clockReads=400 maxClockReads=18 nsPerInstruction=800 stringClones=0 stringCloneBytes=0 borrowedBuiltins=1000 ownedBuiltins=1 procedureCalls=0 localPoolGrows=0 localPoolReuses=0 localInitializations=0 result=OK',
        'R4BASIC performance-workload: name=string-ucase instructions=70000 ticks=1000 ips=70000 slices=30 maxSlice=70000 timeLimited=2 noFixedSleep=1 clockReads=400 maxClockReads=18 nsPerInstruction=1300 stringClones=0 stringCloneBytes=0 borrowedBuiltins=800 ownedBuiltins=1 procedureCalls=0 localPoolGrows=0 localPoolReuses=0 localInitializations=0 result=OK',
        'R4BASIC performance-workload: name=call instructions=100000 ticks=1000 ips=100000 slices=30 maxSlice=100000 timeLimited=2 noFixedSleep=1 clockReads=400 maxClockReads=18 nsPerInstruction=900 stringClones=0 stringCloneBytes=0 borrowedBuiltins=0 ownedBuiltins=0 procedureCalls=1000 localPoolGrows=1 localPoolReuses=999 localInitializations=1000 result=OK',
        'R4BASIC performance-workload: name=array instructions=110000 ticks=1000 ips=110000 slices=30 maxSlice=110000 timeLimited=2 noFixedSleep=1 clockReads=400 maxClockReads=18 nsPerInstruction=850 stringClones=0 stringCloneBytes=0 borrowedBuiltins=0 ownedBuiltins=0 procedureCalls=0 localPoolGrows=0 localPoolReuses=0 localInitializations=0 result=OK',
    ...
    foreach ($invalid in @(
        ...
        $valid.Replace('maxSlice=90000', 'maxSlice=262145'),
